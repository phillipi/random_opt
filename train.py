...
def test(args, models, weights, device, test_loader, train_loader):
    ...
    for batch_idx, (data, target) in enumerate(test_loader):
        ...
        for model_idx, model in enumerate(models):
            ...
            if preds is None:
                preds = output*0.0
            preds += torch.nn.functional.one_hot(output.argmax(dim=1, keepdim=False), num_classes=preds.shape[1])  # a single vote
        print(preds)
...
def main():
    ...
    for epoch in range(1, 100):#args.epochs + 1):
        ...
        for i in range(0,N_models):
            models.append(Net().to(device))
            print('top seed {}: {} (loss: {}, acc: {}%)'.format(i, ii[i]+seed_start_0, losses[ii[i]], accs[ii[i]]))
